...
<style>
  ...
  *{ box-sizing:border-box; }
  ...
  /* The caption itself keeps being drawn on the canvas underneath, so the text
     here is invisible: only the caret and the selection show through. */
  .inline-edit{ position:absolute; resize:none; border:none; outline:2px solid var(--accent); background:rgba(255,255,255,.12);
    padding:0; margin:0; overflow:hidden; color:transparent; caret-color:var(--accent); white-space:pre-wrap; }
  .inline-edit::selection{ background:rgba(124,58,237,.35); color:transparent; }
...
(function(){
  ...
  /* Lays the invisible textarea exactly over the caption the canvas draws:
     same fitted size, same wrap width, same lines. Re-run on every keystroke,
     because shrink-to-fit and the number of lines move as the text changes. */
  function placeInlineEdit(t){
    var lay = NefisBox.measureText(measureCtx, t.default_value || t.placeholder || ' ', renderText(t));
    var h = lay.lines.length * lay.lineHeight;
    var left = t.align === 'left' ? t.x : t.align === 'right' ? t.x - t.max_width : t.x - t.max_width / 2;
    placeBox(inlineEl, { left: left, top: t.y - h / 2, w: t.max_width, h: h,
                         rot: t.rotation || 0, px: t.x, py: t.y });
    inlineEl.style.font = lay.weight + ' ' + (lay.size * zoom) + 'px ' + NefisBox.fontStack(renderText(t));
    inlineEl.style.lineHeight = (lay.lineHeight * zoom) + 'px';
  ...
  function startInlineEdit(index){
    ...
    inlineEl.addEventListener('input', function(){
      ...
      t.default_value = inlineEl.value;
      placeInlineEdit(t);
      render();
      renderProps(true);
    });
    inlineEl.addEventListener('keydown', function(e){
      /* Shift+Enter breaks the line; Enter on its own is done. */
