...
@section('page_style')
  ...
  .wr-card{ background:var(--paper); border:1px solid var(--line); border-radius:var(--radius-sm); overflow:hidden;
  ...
  .wr-body h3{ font-size:.95rem; font-weight:700; }
  .wr-body span{ font-size:.8rem; color:var(--cocoa-soft); white-space:nowrap; }
...
@section('content')
  ...
  <section style="padding-top:0;">
    <div class="wrap">
          ...
          <div class="wr-grid">
            @foreach($wraps as $w)
              <div class="wr-card">
                ...
                <div class="wr-body">
                  <h3>{{ $w['name'] }}</h3>
                  <span>{{ \App\Models\Wrapping::RIBBONS[$w['ribbon']] ?? '' }}</span>
                </div>
              </div>
            @endforeach
          </div>
      ...
      <div class="wr-cta">
        <p>Qablaşdırmanı dizaynı seçəndə, şokoladdan sonra seçirsiniz.</p>
