...
<div class="p-card reveal">
  <a href="{{ $link }}" class="p-card-media">
    ...
    <img src="{{ \App\Support\Media::url($product->catalogImage()) }}" alt="{{ $product->name }} — {{ __('şəkilli şokolad qutusu') }}" loading="lazy">
  </a>
  <div class="p-card-body">
    <h3>{{ $product->name }}</h3>
    <p>{{ $product->description ?: __($product->categoryLabel()) }}</p>
